document.addEventListener('DOMContentLoaded', () => {
    ...
    let lang_list_items = document.querySelector('.header-navigation-lang-items')
    ...
    lang_control.addEventListener('click', (e) => {
        document.querySelector('body').addEventListener('click', (e) => {
            if (e.target !== lang_control && e.target !== lang_current && e.target !== lang_list_items) {
    ...
    document.addEventListener('click', function (event) {
        ...
        if (event.target.classList.value !== "") {
            if (event.target.classList.contains('primary-control-control')){
        ...
        let parent_list_items = event.target.parentElement.children
        for (let i = 0; i < parent_list_items.length; i++) {
            parent_list_items[i].classList.remove('current')
        }
